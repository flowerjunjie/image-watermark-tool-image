...
这是一个基于Web的图片水印工具，支持添加文字水印、平铺水印和图片水印。

## 主要功能

- **多种水印类型**：
  - 文字水印：自定义文字内容、颜色、大小、透明度和旋转角度
  - 平铺水印：设置文字平铺的间距
  - 图片水印：上传图片作为水印
...
- **批量处理**：
  - 支持一次上传和处理多张图片
  - 支持上传整个文件夹并保留目录结构
  - 支持通过按住 Ctrl/Cmd 多选缩略图进行批量下载
  - 使用ZIP格式打包下载处理后的所有图片

- **GIF支持**：
  - 支持为GIF动图添加水印，保留原始动画效果
  - 可调整GIF的质量参数平衡文件大小和质量

- **拖拽定位**：
  - 可以直接拖动水印调整位置
  - 使用鼠标滚轮调整水印大小
  - 设置水印的固定大小或相对于图片大小的比例
...
1. **上传图片**：
   - 点击"上传图片"按钮或将图片拖放到上传区域
   - 支持多选图片同时上传

2. **上传文件夹**：
   - 点击"上传文件夹"按钮选择要上传的文件夹
   - 工具会保留文件夹的目录结构
...
6. **多选和下载**：
   - 单张图片：点击"下载单张图片"按钮下载当前显示的图片
   - 多张图片：按住Ctrl键(Windows)或⌘键(Mac)点击缩略图可以选择多个图片，然后点击"批量下载(ZIP)"
   - 如果没有选择任何缩略图，批量下载将处理所有图片

7. **应用设置到所有图片**：
   - 点击"应用到所有"按钮将当前图片的水印设置应用到所有图片

## 运行环境

本工具为纯前端应用，可以直接在现代浏览器中运行，无需服务器。
