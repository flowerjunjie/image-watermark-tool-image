...
async function gif(buffer) {
  try {
    ...
    try {
      ...
      // 尝试其他可用的库
      if (availableLibraries.includes(GIF_LIBRARIES.GIFUCT)) {
        console.log('尝试使用gifuct-js解析');
        return parseWithGifuct(buffer);
      } else if (availableLibraries.includes(GIF_LIBRARIES.OMGGIF)) {
        console.log('尝试使用omggif解析');
        return parseWithOmggif(buffer);
      } else if (availableLibraries.includes(GIF_LIBRARIES.LIBGIF)) {
        console.log('尝试使用libgif-js解析');
        return parseWithLibgif(buffer);
      } else {
        console.log('尝试使用基本解析方法');
        return parseBasic(buffer);
      }
    } finally {
      // 清理URL
    ...
    return parseBasic(buffer); // 最终回退到基本解析
  }
} 